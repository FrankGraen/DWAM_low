...
from learning.shac.shac import SHAC
from learning.skrl.configure_models import get_models
from utils.config import convert_skrl_cfg

def SHAC_agent(experiment_cfg, observation_space: Box, action_space: Box, env: ManagerBasedRLEnv, conv):

    # Define memory size
    memory_size = experiment_cfg["agent"]["rollouts"]
    memory = RandomMemory(memory_size=memory_size, num_envs=env.num_envs, device=env.device)
    # Get the models
    models = get_models("SHAC", env, observation_space, action_space, conv)
