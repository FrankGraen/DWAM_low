...
from learning.skrl.models import (Critic, DeterministicActor, DeterministicNeuralNetwork,
...
def get_model_gaussian(env: ManagerBasedRLEnv, observation_space: Box, action_space: Box):
    models = {}
    encoder_input_size = None  # 修改：强制设为 None
    mlp_input_size = observation_space.shape[0]  # get the input size from the observation space
...
def get_model_gaussian_conv(env: ManagerBasedRLEnv, observation_space: Box, action_space: Box):
    ...
    mlp_input_size = observation_space.shape[0] - encoder_input_size  # get the input size from the observation space
