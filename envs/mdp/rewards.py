...
def trajectory_following_reward(
    ...
    velocity_alignment_weight: float = 0.5,
    off_track_penalty: float = -1.0,
    backward_penalty: float = -0.5,
    distance_jump_penalty: float = -0.2,
) -> torch.Tensor:
    """
    高级轨迹跟踪奖励，包含惩罚项和平滑性约束
    ...
    Args:
        ...
        distance_weight: 距离奖励权重
        progress_weight: 进度奖励权重
        velocity_alignment_weight: 速度对齐奖励权重
        off_track_penalty: 脱轨惩罚系数
        backward_penalty: 倒退惩罚系数
        smoothness_weight: 平滑性奖励权重
    ...
    # ========== 1. 距离奖励 ==========
    distance = env.trajectory_distance
    distance_reward = torch.clamp(distance_threshold - distance, min=0.0) / distance_threshold
    ...
    if hasattr(env, 'trajectory_progress'):
        ...
        if not hasattr(env, '_prev_trajectory_progress'):
            env._prev_trajectory_progress = torch.zeros(env.num_envs, dtype=torch.float, device=device)
        ...
        progress_reward = torch.where(
            progress_delta > 0,
            progress_delta * 20.0,  # 前进: 正奖励
            progress_delta * 30.0   # 倒退: 更大惩罚
        )
        
        total_reward += progress_weight * progress_reward
        # env._prev_trajectory_progress = progress.clone()
    
    # ========== 3. 速度对齐奖励 ==========
    if hasattr(env, 'trajectory_closest_points'):
        box_vel = env.scene[box_name].data.root_link_vel_w[:, :2]
        box_speed = torch.norm(box_vel, dim=-1, keepdim=True) + 1e-6
        
        if hasattr(env, '_prev_closest_points'):
            
            closest_idx = env.closest_point_index  # 当前离盒子最近的轨迹点索引
            lookahead_steps = 3  # 向前看3个点
            target_idx = torch.clamp(closest_idx + lookahead_steps, max=len(env.current_trajectories[0])-1)
            env_indices = torch.arange(env.num_envs, device=env.device)
            target_point = env.current_trajectories[env_indices, target_idx]  # (num_envs, 3)
            ...
            total_reward += velocity_alignment_weight * velocity_reward
        
        # env._prev_closest_points = env.trajectory_closest_points.clone()
    
    # ========== 4. 脱轨惩罚 ==========
    # 如果距离超过阈值的2倍，给予惩罚
    off_track_mask = distance > (distance_threshold * 3.0)
    off_track_penalty_value = off_track_penalty * (distance - distance_threshold * 3.0)
    total_reward[off_track_mask] += off_track_penalty_value[off_track_mask]
    
    # ========== 5. 倒退惩罚 ==========
    if hasattr(env, 'trajectory_progress'):
        backward_mask = progress_delta < -0.02  # 小的波动不算倒退
        backward_mask = backward_mask & ~is_loop  # 排除循环情况
        total_reward[backward_mask] += backward_penalty
    
    # ========== 6. 跳跃惩罚 ==========
    # 惩罚剧烈的距离变化，鼓励稳定跟踪
    if hasattr(env, '_prev_trajectory_distance'):
        distance_change = torch.abs(distance - env._prev_trajectory_distance)
        jump_penalty_value = distance_jump_penalty * distance_change
        total_reward += jump_penalty_value
    
    # 更新历史距离
    # env._prev_trajectory_distance = distance.clone()
    
    # 根据是否完成reach_box任务发放奖励
    total_reward[~env.reached_box_flags] = 0.0       
    return total_reward
