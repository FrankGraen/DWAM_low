...
parser.add_argument("--task", type=str, default="BoxPushEnv-v0", help="Name of the task.")
...
parser.add_argument("--save_cam", action="store_true",
                    help="Save images from the follow‑cam (PNG sequence)")
parser.add_argument("--cam_res", type=int, nargs=2, default=[800, 450],
                    metavar=("W", "H"), help="Follow‑cam resolution")
parser.add_argument("--follow_cam", action="store_true",
                    help="Activate a third‑person camera that tracks env0’s robot.")
...
# Import agents
from learning.skrl import get_agent  # noqa: E402
from utils.config import parse_skrl_cfg  # noqa: E402
...
def main():
    ...
    # Check if follow camera is enabled
    env.reset()                                           #    
    if args_cli.follow_cam:
        stage     = omni.usd.get_context().get_stage()
        base_link = "/World/envs/env_0/Robot/base_link"
        cam_path  = base_link + "/follow_cam"

        # create the follow‑cam if it does not exist
        ...
        xf.SetTranslate((0.0, 0.0, 3.0))
        xf.SetRotate((0.0, 0.0, 0.0), UsdGeom.XformCommonAPI.RotationOrderXYZ)
        cam = UsdGeom.Camera(stage.GetPrimAtPath(cam_path))
        # cam.GetFocalLengthAttr().Set(1.0)                   # 
        # 
        # https://forums.developer.nvidia.com/t/the-camera-crops-objects-when-i-zoom-in/220278/3
        # https://docs.omniverse.nvidia.com/kit/docs/pxr-usd-api/105.0.2/pxr/UsdGeom.html#pxr.UsdGeom.Camera
        cam.GetClippingRangeAttr().Set(Gf.Vec2f(0.01, 1000))
        W, H = args_cli.cam_res
        rp   = rep.create.render_product(cam_path, (W, H))
        
        # save image to dir and then convert to video
        # not elegent and not completed yet
        '''
        if args_cli.save_cam:
        save_dir = os.path.join(log_dir, "follow_cam_png")
        writer   = rep.WriterRegistry.get("BasicWriter")
        writer.initialize(output_dir=save_dir, rgb=True)
        writer.attach(rp)
        print(f"[INFO] Follow‑cam frames → {save_dir}")
        '''
    ...
    agent = get_agent(args_cli.agent, env, observation_space, action_space, experiment_cfg, conv=None)

    # Get the checkpoint path from the experiment configuration
    print(f'args_cli.task: {args_cli.task}')
    # agent_policy_path = gym.spec(args_cli.task).kwargs.pop("best_model_path")#TODO: load model from argparse rather than gym spec
    agent_policy_path = args_cli.checkpoint
